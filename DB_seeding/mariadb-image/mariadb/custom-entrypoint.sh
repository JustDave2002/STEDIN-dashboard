...
# Start MariaDB with --skip-grant-tables option as the 'mysql' user
exec mysqld --skip-grant-tables  --user=mysql &
pid="$!"

# Wait for the server to start
sleep 10

# Run the SQL commands to initialize the database (create the database, user, etc.)
mysql -u root <<-EOSQL
  CREATE DATABASE IF NOT EXISTS edge_management;
...
# Wait for mysqld process to exit
wait "$pid"
